...
void scientific(vector<vector<misura>> &m, vector<string> &v){
    cout << "Inserire quali misure vanno i notazione scientifica (es. yes/no):" << endl;
    string tmp;
...
//Computing
void oldorder(std::vector<std::vector<misura>> &m) {
    for (int i = 0; i < m.size(); i++) {
        for (int k = 0; k < m[i].size(); k++) {
            // Se yn è "yes", applica la notazione scientifica
            if (m[i][k].yn == "yes") {
                m[i][k].ord_m=static_cast<int>(log10(m[i][k].mis));
                m[i][k].ord_e=static_cast<int>(log10(m[i][k].err));
            }else{
                m[i][k].ord_m = 0;
                m[i][k].ord_e = 0;
            }
            if(m[i][k].ord_m<0){
                m[i][k].ord_m=m[i][k].ord_m-1;
            }else{
                m[i][k].ord_m=m[i][k].ord_m+1;
            }
            if(m[i][k].ord_e<0){
                m[i][k].ord_e=m[i][k].ord_e-1;
            }else{
                m[i][k].ord_e=m[i][k].ord_e+1;
            }
        }
    }
    cout << m[0][0].ord_m << endl;
    cout << m[0][0].ord_e << endl;
}
void newmis(std::vector<std::vector<misura>>& m) {
    for (int i = 0; i < m.size(); i++) {
        for (int k = 0; k < m[i].size(); k++) {
            // Controlla se yn è "yes", solo in questo caso esegui la conversione
            if (m[i][k].yn == "yes") {
                int ord_m_diff=0;
                if(m[i][k].ord_m<0 && m[i][k].new_o<0 && m[i][k].ord_m<m[i][k].new_o){
                    ord_m_diff=abs(m[i][k].ord_m-m[i][k].new_o);
                }else if(m[i][k].ord_m<0 && m[i][k].new_o<0 && m[i][k].ord_m>m[i][k].new_o){
                    ord_m_diff=abs(-m[i][k].ord_m+m[i][k].new_o);
                }else if(m[i][k].ord_m<0 && m[i][k].new_o>0){
                    ord_m_diff=abs(-m[i][k].ord_m+m[i][k].new_o);
                }else if(m[i][k].ord_m>0 && m[i][k].new_o<0){
                    ord_m_diff=abs(m[i][k].ord_m-m[i][k].new_o);
                }else if(m[i][k].ord_m>0 && m[i][k].new_o>0 && m[i][k].ord_m>m[i][k].new_o){
                ...
                int ord_e_diff=0;
                if(m[i][k].ord_e<0 && m[i][k].new_o<0 && m[i][k].ord_e<m[i][k].new_o){
                    ord_e_diff=abs(m[i][k].ord_e-m[i][k].new_o);
                }else if(m[i][k].ord_e<0 && m[i][k].new_o<0 && m[i][k].ord_e>m[i][k].new_o){
                    ord_e_diff=abs(-m[i][k].ord_e+m[i][k].new_o);
                }else if(m[i][k].ord_e<0 && m[i][k].new_o>0){
                    ord_e_diff=abs(-m[i][k].ord_e+m[i][k].new_o);
                }else if(m[i][k].ord_e>0 && m[i][k].new_o<0){
                    ord_e_diff=abs(m[i][k].ord_e-m[i][k].new_o);
                }else if(m[i][k].ord_e>0 && m[i][k].new_o>0 && m[i][k].ord_e>m[i][k].new_o){
                ...
                cout << ord_m_diff << endl;
                cout << ord_e_diff << endl;
                if(ord_m_diff!=0){
                    if(m[i][k].ord_m<0 && m[i][k].new_o<0 && m[i][k].ord_m<m[i][k].new_o){
                        m[i][k].mis=m[i][k].mis*pow(10, -ord_m_diff);
                    }else if(m[i][k].ord_m<0 && m[i][k].new_o<0 && m[i][k].ord_m>m[i][k].new_o){
                        m[i][k].mis=m[i][k].mis*pow(10, ord_m_diff);
                    }else if(m[i][k].ord_m<0 && m[i][k].new_o>0){
                        m[i][k].mis=m[i][k].mis*pow(10, -ord_m_diff);
                ...
                if(ord_e_diff!=0){
                    if(m[i][k].ord_e<0 && m[i][k].new_o<0 && m[i][k].ord_e<m[i][k].new_o){
                        m[i][k].mis=m[i][k].mis*pow(10, -ord_e_diff);
                    }else if(m[i][k].ord_e<0 && m[i][k].new_o<0 && m[i][k].ord_e>m[i][k].new_o){
                        m[i][k].mis=m[i][k].mis*pow(10, ord_e_diff);
                    }else if(m[i][k].ord_e<0 && m[i][k].new_o>0){
                        m[i][k].mis=m[i][k].mis*pow(10, -ord_e_diff);
